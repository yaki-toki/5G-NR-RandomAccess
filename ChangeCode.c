...
void preambleCollisionDetection(struct UEinfo *user, int activeCheck, int time, int *grantCheck);
...
int main(int argc, char **argv){
    ...
    int nGrantUL = 54;         // Number of UL Grant
    ...
    for (randomSeed = 0; randomSeed < 1; randomSeed++){
        ...
        for (int n = 10000; n <= 100000; n += 10000){
            ...
            for (time = 0; time < maxTime; time++){
                ...
                preambleCollisionDetection(UE, activeCheck, time, &grantCheck);
...
void preambleCollisionDetection(struct UEinfo *user, int activeCheck, int time, int *grantCheck){
    ...
    //(UE + i)->active == 1 && (UE + i)->msg2Flag == 0 && (UE + i)->txTime == time

    // Store idx of UEs attempting to send MSG 1 at the current time
    for(int i = 0; i < activeCheck; i++){
        if((user+i)->active == 1 && (user+i)->msg2Flag == 0 && (user+i)->txTime == time){
                ...
                *grantCheck = *grantCheck + 1;
                for(int j = 0; j < preambles[i]; j++){
                    if(*grantCheck <= nGrantUL){
                        (user+userCheck[i][j])->msg2Flag = 1;
                        (user+userCheck[i][j])->txTime = time + 11;
                        (user+userCheck[i][j])->connectionRequest = 0;
                        (user+userCheck[i][j])->active = 2;
                    }else{
                        (user+userCheck[i][j])->txTime++;
                    }
                }
